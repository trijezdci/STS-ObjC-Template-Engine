...
#define keyDefined(x,y) ([x objectForKey:y] != nil)
#define keyNotDefined(x,y) ([x objectForKey:y] == nil)
...
- (NSString *)stringByExpandingPlaceholdersWithStartTag:(NSString *)startTag
										...
										usingDictionary:(NSDictionary *)dictionary
										 errorsReturned:(NSArray **)errorLog
											 lineNumber:(unsigned)lineNumber
{	
	NSMutableString *remainder = [NSMutableString stringWithCapacity:[self length]];
	...
	if found(tag) {
		...
		while found(tag) {
			...
			// remove preceeding text and start tag from the remainder
			range.location = 0; range.length = tag.location+tag.length;
			[remainder deleteCharactersInRange:range];
			// look for the end tag
			tag = [remainder rangeOfString:endTag];
			// if we did find the end tag ...
			if found(tag) {
				// extract the placeholder
				[placeholder setString:[remainder substringToIndex:tag.location]];
				// use placeholder as key for dictionary lookup
				value = [dictionary objectForKey:placeholder];
				...
				// if the lookup returns a value for the key ...
				  ...
				  // remove placeholder and end tag from the remainder
				range.location = 0; range.length = tag.location+tag.length;
				[remainder deleteCharactersInRange:range];
			} // end if
			  // if we don't find any end tag ...
				...
				[remainder setString:kEmptyString];
				// this is an error - create a new error description
		...
		// if there are still characters in the remainder ...
...
@interface TEFlags : NSObject {
	...
	@public unsigned consumed, expand, condex;
} // end var
//
				 ...
				 errorsReturned:(NSArray **)errorLog
{
	...
	NSString *line, *remainder, *keyword, *key, *value, *operand;
	unsigned len, lineNumber = 0;
	...
	while (line = [list nextObject]) {
		...
		if (([line hasPrefix:startTag] == NO) && ([line characterAtIndex:0] == '%')) {
			...
			if (([keyword isEqualToString:@"%IF"]) || ([keyword isEqualToString:@"%IFNOT"])) {
				if (flags->expand) {
					...
					// if there is an identifier following %IF/%IFNOT ...
						...
						value = [_dictionary valueForKey:key];
			...
			else if (([keyword isEqualToString:@"%ELSIF"]) || ([keyword isEqualToString:@"%ELSIFNOT"])) {
				if (flags->condex) {
			...
			else if (([keyword isEqualToString:@"%IFEQ"]) || ([keyword isEqualToString:@"%IFNEQ"])) {
				if (flags->expand) {
					...
					// if there is an identifier following %IFEQ/%IFNEQ ...
						...
						remainder = [[line restOfWordsUsingDelimitersFromSet:whitespaceSet]
						...
						if (len == 0) {
							...
							if (len == 1) {
								...
								if ([remainder characterAtIndex:0] == '\'') {
									...
									operand = [remainder substringWithStringInSingleQuotes];
									// if there are no closing quotes
									if (operand == nil) {
										// assume EOL terminates the quoted string
										operand = [remainder substringFromIndex:1];
									} // end if
								}
								// alternatively, a quoted string using double quotes
								else if ([remainder characterAtIndex:0 == '"']) {
									// get the characters enclosed by the double quotes
									operand = [remainder substringWithStringInDoubleQuotes];
									// if there are no closing quotes
									if (operand == nil) {
										// assume EOL terminates the quoted string
										operand = [remainder substringFromIndex:1];
								...
								// otherwise if we don't have a quoted string
									...
									operand = [remainder firstWordUsingDelimitersFromSet:whitespaceSet];
								} // end if
							} // end if
							// *** this is the surviving branch - others are error branches ***
							// if this is a nested if-else block ...
			...
			if (([keyword isEqualToString:@"%ELSIFEQ"]) || ([keyword isEqualToString:@"%ELSIFNEQ"])) {
			...
			else if (([keyword isEqualToString:@"%IFDEF"]) || ([keyword isEqualToString:@"%IFNDEF"])) {
			...
			else if (([keyword isEqualToString:@"%ELSIFDEF"]) || ([keyword isEqualToString:@"%ELSIFNDEF"])) {
				if (flags->condex) {
					// if any branch in this if-else block was true ...
					if (flags->consumed) {
						// do not evaluate
			...
			else if ([keyword isEqualToString:@"%ELSE"]) {
				if (flags->condex) {
					// if any branch in this if-else block was true ...
			...
			else if ([keyword isEqualToString:@"%ENDIF"]) {
				if (flags->condex) {
			...
			else if ([keyword isEqualToString:@"%DEFINE"]) {
				if (flags->expand) {
			...
			else if ([keyword isEqualToString:@"%UNDEF"]) {
				if (flags->expand) {
			...
			else if ([keyword isEqualToString:@"%LOG"]) {
				if (flags->expand) {
			...
			else if ([keyword isEqualToString:@"%ECHO"]) {
				if (flags->expand) {
			...
			else if ([keyword isEqualToString:@"%DEBUG"]) {
				if (flags->expand) {
					   ...
					   errorsReturned:(NSArray **)errorLog
{
	...
	NSLog(@"STS Template Engine - expanding template file %@ ...", path);
	return [NSString stringByExpandingTemplate:templateString
